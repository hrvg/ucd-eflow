...
Ac.iterate <- read.csv(file.path(csv.root, "area_min_max_less1000.csv"), header=TRUE, sep=",")
S.iterate <- read.csv(file.path(csv.root, "slope_min_max.csv"), header=TRUE, sep=",")
...
for (i in 1:length(Ac.iterate[,1])) {
	for (j in 1:length(S.iterate[,1])) {
		...
		if (lsr.rows > 10) {lsr.yn <- 1} else {lsr.yn <- 0}
		if (rsg.rows > 10) {rsg.yn <- 1} else {rsg.yn <- 0}
		level.num <- sum(lsr.yn, rsg.yn)
		
		# Recombine for MANOVA analysis
		hydro.AcS <- as.matrix(rbind(lsr.AcS[,1:13], rsg.AcS[,1:13]))
		maov.hydro.AcS <-as.matrix(hydro.AcS[,3:13])

		if (level.num > 1 & nrow(maov.hydro.AcS) > 15) {
			...
			aov.hydro.AcS <- maov.hydro.AcS
			
			# MANOVA
			aggregate(maov.hydro.AcS, by=list(hydro.class), FUN=mean)
			cov(maov.hydro.AcS)
			manova.fit <- manova(maov.hydro.AcS ~ hydro.class) 
			p.manova <- as.numeric(summary(manova.fit)$stats[1, "Pr(>F)"])
			...
			gm.atts <- as.data.frame(hydro.AcS)
			...
			if (stats.num == 1) {
				...
				bf.w.d.out <- cbind(mean(gm.atts$bf.w.d[hydro.class=="LSR"]),
									mean(gm.atts$bf.w.d[hydro.class=="RSG"]))
				area.out <- cbind(mean(gm.atts$Ac[hydro.class=="LSR"]),
									mean(gm.atts$Ac[hydro.class=="RSG"]))
			} else {
				...
				bf.w.d.out <- rbind(bf.w.d.out,(cbind(mean(gm.atts$bf.w.d[hydro.class=="LSR"]),
									mean(gm.atts$bf.w.d[hydro.class=="RSG"]))))
				area.out <- rbind(area.out,(cbind(mean(gm.atts$Ac[hydro.class=="LSR"]),
									mean(gm.atts$Ac[hydro.class=="RSG"]))))
			}
