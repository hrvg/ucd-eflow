...
if(paste(method)=="evans"){
	crop(focal(data,w=matrix(1/9, nc=3, nr=3),fun=evans,pad=T,padValue=0),y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))
}
else{
	if(paste(method)=="zev.tho"){
		crop(focal(data,w=matrix(1/9, nc=3, nr=3),fun=zev.tho,pad=T,padValue=0),y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))
	}
	else{
		if(paste(method)=="shary"){
			crop(focal(data,w=matrix(1/9, nc=3, nr=3),fun=shary,pad=T,padValue=0),y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))
		}
		else{
			if(paste(method)=="moore"){
				crop(focal(data,w=matrix(1/9, nc=3, nr=3),fun=moore,pad=T,padValue=0),y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))
			}
		}
	}	
}
...
#REFERENCES
#Evans, I. S. (1980). An Integrated System of Terrain Analysis and Slope Mapping. Zeitschrift f?r Geomorphologie, Suppl. Bd. 36, 274-295.
